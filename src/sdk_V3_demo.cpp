...
int main()
{
	...
	int iBaud = 150000;// getBaud();

	std::string strLidarModel = D2YE;// getLidarModel();
	...
	if (strLidarModel == D2YE)
	{
		setSDKYJStart(false);
		g_strLidarID = getSDKDYJSn();
		if (g_strLidarID.empty())
		{
			hcSDKUnInit();
			printf("Main: Get lidar SN error!\n");
			getchar();
			exit(0);
			return 0;
		}
		printf("Main:Lidar SN=%s\n", g_strLidarID.c_str());

		if (!setSDKYJStart(true))
		{
			hcSDKUnInit();
			printf("Main: Start lidar error!\n");
			getchar();
			exit(0);
			return 0;
		}
